...
def collate_vton_fn(data):
    ...
    image_garm = torch.Tensor([example["image_garm"] for example in data])
    image_vton = torch.Tensor([example["image_vton"] for example in data])
    mask = torch.Tensor([example["mask"] for example in data])
    image_ori = torch.Tensor([example["image_ori"] for example in data])
    clip_ref_image = torch.cat([example["clip_ref_image"] for example in data])
...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        cloth_img = Image.open(osp.join(self.data_path, "cloth", c_name[key])).resize((768, 1024))
        model_img = Image.open(osp.join(self.data_path, im_name)).resize((768, 1024))
        keypoints = self.openpose_model(model_img.resize((384, 512)))
        model_parse, _ = self.parsing_model(model_img.resize((384, 512)))

        mask, mask_gray = get_mask_location("dc", 'upper_body', model_parse, keypoints)
